...
def main():
    ...
    while cap.isOpened():
        ...
        if len(objs) != 0:
            #print('objs',objs)
            cv2_im = append_objs_to_img(cv2_im, inference_size, objs, labels)  # if any object is added pass it to func to draw results on frame
...
def append_objs_to_img(cv2_im, inference_size, objs, labels):
    ...
    if objs:
        ...
        new_objs = []  # to store only person and car results

        for e,obj in enumerate(objs):

            if obj.id==0 or obj.id==2:
            ...
            if len(new_objs) != 0:
                print('Total 1st detetcted objects: ', len(new_objs))
                ...
                print('Detecttions total: ', detection,detection.shape)
                det = detection.copy()
                tr_update = mot_tracker.update(det)  # now assigning id through sort
                # tr_update2 = tr_update.copy()
                # print('sort result:', tr_update)
                print('After sort: ',tr_update)
                if len(tr_update)!=0:
...
def center_coord(detection_data,idx):
    ...
    print("idx------------",idx)
    # target_index = detection_data[0]
    # target_index = [int(target_index[4])]
